...
from .cleaner import clean_dataset_text
...
from ..utils.io import ensure_parent, write_json
...
def preprocess_dataset(
    ...
    参数:
        ...
        limit:          最多保留多少条片段(用于小规模试跑);None 表示不限。
    ...
    with output.open("w", encoding="utf-8", newline="\n") as f, error_path.open("w", encoding="utf-8", newline="\n") as ef:
        for raw in tqdm(read_local_records(dataset, input_path), desc=f"preprocess {dataset}", unit="doc"):
            ...
            for frag_idx, fragment in enumerate(fragments):
                ...
                record_idx += 1
                counters["kept"] += 1
                # 达到 limit 上限就停(内层 break)。
                if limit is not None and record_idx >= limit:
                    LOGGER.info("Reached limit for %s: %s", dataset, limit)
                    break
            # 达到上限再跳出外层文档循环。
            if limit is not None and record_idx >= limit:
                break
